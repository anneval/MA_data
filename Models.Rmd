...
- AR(1) and standard OLS (split data IS and OOS) (5x)
- AR(1) and expanding estimation  (IS and OOS) - party missing (5x) - cannot compare to others sample set
- auto.arima model (lag selection automatic, and 4)
- VAR model (lag selection automatic, and 4)
...
```{r}
# descriptives? do for all 3 ... and graphs 
...
```{r}
v <- VAR(Data[,c(1,3)], type="const", p=2)
#v <- vecm16var
#v <- vecm16
horizon= 3
...
library(Metrics)
x <- rmse(Data[,1],p$fcst[[1]][,"fcst"]) #1 for gdp 
x

for(i in 1:3){
  fc <- structure(list(mean=p$fcst[[i]][,"fcst"], x=Data_train[,i],
                       fitted=c(rep(NA,lags),fits[,i])),class="forecast")
  acc_pes <- accuracy(fc,Data_test[,1])
  acc_Dune <- accuracy(fc,Data_test[,2])
  acc_cpi <- accuracy(fc,Data_test[,3])
}
```
...
(0,0,0) loop - later if time... not needed dont want best model gdp want impact of google in gdp forecast
```{r}
