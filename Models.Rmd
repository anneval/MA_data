...
VAR1_est <- forecast::var(y = Data[,1:3], p = lag_VAR1,type = "const")
...
```{r}
MDiagnostics_table <- cbind(ser.test_VAR1_PT$serial,ser.test_VAR_BG$serial,archtestVAR1$arch.mul,Normtest_VAR1$jb.mul$JB) 

MDiagnostics_table.names <- rbind("Chi-squared
","df","p-values", "method","data.name
"
                        )  
...
midas1 <-lm(y~trend+mls(x,k=0:7,m=4)+mls(z,k=0:16, m=12))
...
sqrt(avgf$accuracy$individual$MSE.out.of.sample) # RMSE
```

MIDAS trial GDP und CCI
```{r}
