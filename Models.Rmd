...
Data$cci_ts <- ts(Data$cci, start=c(2006,1), end = c(2023,1), frequency = 4)
...
Data$google_ts <- ts(Data$google, start=c(2006,1), end = c(2023,1), frequency = 4)
```
...
Data_train <- Data[1:48,] # 70%
Data_test <- Data[49:69,] #30%
```
...
adfcci <- adf.test(Data$cci) # not stationary
...
expanding_window_OLS <- function(data, dep_var, start = 20){ 
  expanding_OLS <- list() # empty vector
  predicted_OLS <- list()
  resid_OLS <- list()
  error_OLS <- c() 
  i <- 0
  for(t in start:nrow(data)){ # from 19 bis 63, neu 20-69
    i <- i+1 # first one is i=1 
    expanding_OLS[[i]] <- lm(formula = model, data = data[1:(t-1),]) # first train is 1:18, 1:19 neu
...
expanding_OLS1 <- expanding_window_OLS(Data[2:69,], dep_var, start = 20)
...
pred1 <- as.ts(pred1, start=c(2011,1), end = c(2023,1), frequency = 4)
...
```{r}
plotexp_1 <- ts.plot(ts(Data[21:69,1],frequency = 4, start = c(2011,1),end = c(2023,1)),
          ts(pred1,frequency = 4, start = c(2011,1),end = c(2023,1)),
    col = c("black", "red"),
    xlab = "Year", 
    ylab = "GDP growth (%)", 
    lwd = 1)
...
```{r, message=FALSE, warning=FALSE}
model = gdp ~ cci
expanding_OLS2 <- expanding_window_OLS(Data, dep_var, start = 21)

pred2 <- do.call(rbind.data.frame, expanding_OLS2[[2]]) 
colnames(pred2) <- c("value")
Date_q_pred <- Date_q[21:69]

pred2 <- as.ts(pred2,start=c(2011,1), end = c(2023,1), frequency = 4)
...
```{r, message=FALSE, warning=FALSE}
plotexp_2 <- ts.plot(ts(Data[21:69,1],frequency = 4, start = c(2011,1)),
          ts(pred2,frequency = 4, start = c(2011,1)),
    col = c("black", "red"),
    xlab = "Year", 
    ylab = "GDP growth (%)", 
    lwd = 1)
...
```{r, message=FALSE, warning=FALSE}
model = gdp ~ google
expanding_OLS3 <- expanding_window_OLS(Data, dep_var, start = 21)

pred3 <- do.call(rbind.data.frame, expanding_OLS3[[2]]) 
pred3
colnames(pred3) <- c("value")
Date_q_pred <- Date_q[21:69]
pred3 <- as.ts(pred3, start=c(2011,1), end = c(2023,1), frequency = 4)
...
```{r}
plotexp_3 <- ts.plot(ts(Data[21:63,1],frequency = 4, start = c(2011,4)),
          ts(pred3,frequency = 4, start = c(2011,4)),
    col = c("black", "red"),
    xlab = "Year", 
    ylab = "GDP growth (%)", 
    lwd = 1)
...
model = gdp ~ gdp_lag + cci
  expanding_OLS4 <- expanding_window_OLS(Data[2:69,], dep_var, start = 20)

#predictions
pred4 <- do.call(rbind.data.frame, expanding_OLS4[[2]]) # NAS wegen lag! 
Date_q_pred <- Date_q[21:69]
colnames(pred4) <- c("value")
pred4 <- as.ts(pred4, start=c(2011,1), end = c(2023,1), frequency = 4)
...
expanding_OLS5 <- expanding_window_OLS(Data[2:69,], dep_var, start = 20)

#predictions
pred5 <- do.call(rbind.data.frame, expanding_OLS5[[2]]) # NAS wegen lag! 
colnames(pred5) <- c("value")
Date_q_pred <- Date_q[21:69]
pred5 <- as.ts(pred5, start=c(2011,1), end = c(2023,1), frequency = 4)
...
```{r}
plotexp_5 <- ts.plot(ts(Data[21:69,1],frequency = 4, start = c(2011,1)),
          ts(pred4,frequency = 4, start = c(2011,1)),
          ts(pred5,frequency = 4, start = c(2011,1)),
...
expanding_OLS6 <- expanding_window_OLS(Data[2:69,], dep_var, start = 20)

#predictions
pred6 <- do.call(rbind.data.frame, expanding_OLS6[[2]]) # NAS wegen lag! 
colnames(pred6) <- c("value")
Date_q_pred <- Date_q[21:69]
pred6 <- as.ts(pred6, start=c(2011,1), end = c(2023,1), frequency = 4)
...
expanding_OLS7 <- expanding_window_OLS(Data, dep_var, start = 21)

#predictions
pred7 <- do.call(rbind.data.frame, expanding_OLS7[[2]]) # NAS wegen lag! 
colnames(pred7) <- c("value")
Date_q_pred <- Date_q[21:69]
pred7 <- as.ts(pred7, start=c(2011,1), end = c(2023,1), frequency = 4)
...
```{r}
plotexp_7 <- ts.plot(ts(Data[21:69,1],frequency = 4, start = c(2011,1)),
          ts(pred6,frequency = 4, start = c(2011,1)),
          ts(pred7,frequency = 4, start = c(2011,1)),
    col = c("black", "red","blue"),
    xlab = "Year", 
    ylab = "GDP growth (%)", 
    lwd = 1)
...
expanding_VAR2 <- expanding_window_OLS(Data_Var[3:69,], dep_var, start = 19) 
#predictions
