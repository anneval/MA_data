...
expanding_window_OLS <- function(data, dep_var, start = 19){ 
  ...
  for(t in start:nrow(data)){ # go from 19 bis 63
    ...
    expanding_OLS[[i]] <- lm(formula = model, data = data[1:(t-1),]) # first train is 1:19
    resid_OLS[[i]] <- resid(expanding_OLS[[i]])
    predicted_OLS[i] <- predict(expanding_OLS, newdata = data[t,])### chmage herenewdatda for forecast.. aber nur 3 observationen...
...
try2 <-try[19:62,] # q4 2010
try_1 <-Data[19:63,]# q4 2010
try <-Data[2:63,] 
#predictions
...
#rownames(pred1) <- Date_q_pred[2:51]
...
# Plot model1 against GDP growth
```{r}
#par(mfrow = c(3,1), mar = c(0.5, 4, 4, 4))
...
# Plot model2 against GDP growth
```{r, message=FALSE, warning=FALSE}
...
```
```{r}
checkresiduals(expanding_OLS1[[1]]$residuals)
```

# Plot model3 against GDP growth
```{r}
...
#rownames(pred4) <- Date_q_pred[2:51]
...
# Plot model4 against GDP growth
```{r}
plotexp_4 <- ts.plot(ts(Data[20:63,1],frequency = 4, start = c(2010,4)),
          ts(pred2[2:45,],frequency = 4, start = c(2010,4)), # KÜRZEN!
          ts(pred4,frequency = 4, start = c(2010,4)),
    col = c("black", "red", "blue"),
    xlab = "Year", 
    ylab = "GDP growth (%)", 
    lwd = 1)
legend("bottomleft", c("GDP growth","CCI","CCI & AR(1)"), col= c("black", "red", "blue"),lty=c(1), xpd=TRUE, horiz=, bty="b",cex=0.75
      )
```
...
#rownames(pred5) <- Date_q_pred[2:51]
Date_q_pred <- Date_q[20:63]
...
# Plot model5 against GDP growth
```{r}
...
#rownames(pred6) <- Date_q_pred[2:51]
Date_q_pred <- Date_q[20:63]
...
# Plot model6 against GDP growth
```{r}
plotexp_6 <- ts.plot(ts(Data[20:63,1],frequency = 4, start = c(2010,4)),
          ts(pred6,frequency = 4, start = c(2010,4)),
    col = c("black", "red"),
    xlab = "Year", 
    ylab = "GDP growth (%)", 
    lwd = 1)
legend("bottomright", c("GDP growth", "AR(1) & CCI & PES"), col=c("red","black"), lty=c(1),
       inset=c(0,1), xpd=TRUE, horiz=TRUE, bty="n"
       )
```
Model 7 CCI and PES
...
# here only 3 steps ahead or entire test data set predicted??
