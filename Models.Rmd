...
TO DO: 
- schreiben extension models, results, robustness 
- residual tests on all models (std.ols and arima left)
- overview of results...! tables (OLS (nur robustness), Expanding, AR, VAR) FINALE ..!
- Compare OeNB, WIfo index - Discussion
- consumer index - robustness forecast mit other google indices?

Question: 
- is expanding window correct?
- non pseeudo-forecast aus expanding?
...
stargazer::stargazer(acc_is_MA1,acc_oos_MA1) #arima !!!!! 
...
ts.plot(Data$gdp, forecast_MA1$pred, lty = c(1,3), col=c(5,2))
...
#forecast::autoplot(forecast_VAR1)
...
Box.test(resid_VAR1$VAR1_est.varresult.gdp.residuals, type="Ljung-Box") # cannot rejet H0 The residuals are independently distributed.Ideally, we would like to fail to reject the null hypothesis. That is, we would like to see the p-value of the test be greater than 0.05 because this means the residuals for our time series model are independent, which is often an assumption we make when creating a mode
...
#breusch_godfrey
ser.test_VAR_BG <- serial.test(VAR1_est,type="BG") # H0 (null hypothesis): There is no autocorrelation at any order less than or equal to p.
...
plot(archtestVAR1)
checkresiduals(VAR1_est$varresult$gdp) # only unvariate... 
```
```{r}
...
plot(VAR_Est_CUSUM)
```
### Summary Results Model diagnostics
```{r}
...
RMSE_AR2_IS
```
#### Weekly indicators OeNB and WIFO
...
fcst_midas1 <- midasr::forecast(midas1, method = "static")
fcst_midas1
#acc_oos_m2 <- forecast::accuracy(fcst_midas2$mean,gdp)
#plot(forecast1)

#acc_modmid2 <- rbind(acc_is_m2,acc_oos_m2)
...
## MORE CORRECT VERSION WITH 91.30....days but forecast not possible..
...
```{r}
#63*3 = 189 month
#189month*30 = 5670 days (2021-07-10) (90n)

#2021-09-11 bei 5733 (91)
...
#as.data.frame(Google_AT_daily$value[5753:5919])
...
#Midas regression
midas4 <- midas_r(gdp ~ mls(gdp, 1, 1) + mls(cci, 0, 3), #same as lag(gdp)
              start = NULL)
...
MIDAS model lag(gdp) + google (mod5)

```{r} 
#Midas regression
midas5 <- midas_r(gdp ~ mls(gdp, 1, 1) + mls(google, 0, 91),start = NULL) #same as lag(gdp)
...
MIDAS model lag(gdp) + google + cci (mod6)

```{r} 

#Midas regression
midas6 <- midas_r(gdp ~ mls(gdp, 1, 1) + mls(cci, 0, 3) + mls(google, 0, 91), #same as lag(gdp)
              start = NULL)
...
#### forecasts:
newdata <- list(cci_fcst,google_fcst)
fcst_midas6 <- midasr::forecast(midas6,newdata = newdata, method = "static")
fcst_midas6
```


MIDAS model cci+ google (mod7)

```{r} 

#Midas regression
midas7 <- midas_r(gdp ~ mls(cci, 0, 3) + mls(google, 0, 91), #same as lag(gdp)
              start = NULL)
