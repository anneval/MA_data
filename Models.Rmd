...
### ist noch kein forecast...doch psequdo oos forecast 
```{r}
...
expanding_window_OLS <- function(data, dep_var, start = 13){ # change to 12 
  ...
  for(t in start:nrow(data)){
    i <- i+1
    expanding_OLS[[i]] <- lm(formula = model, data = data[1:(t-1),])
    resid_OLS[[i]] <- resid(expanding_OLS[[i]])
...
pred1 <- as.ts(pred1, start=c(2010,3), end = c(2021,3), frequency = 4)
ts.plot(pred1)
...
x<-Data[20:63,1]
ts.plot(ts(Data[20:63,1],frequency = 4, start = c(2010,3)),
          ts(pred1,frequency = 4, start = c(2010,3)),
    col = c("red", "blue"),
    xlab = "Year", 
    ylab = "GDP growth", 
    lwd = 1)
legend("bottomright", c("GDP growth", "AR(1)"), col=c("red","blue"), lty=c(1),
       inset=c(0,1), xpd=TRUE, horiz=TRUE, bty="n"
       )
```
...
ts.plot(ts(Data[19:63,1],frequency = 4, start = c(2010,3)),
    ...
    ylab = "Search Frequency (Wirtschaftskrise)", 
    lwd = 1)
legend("bottomright", c("GDP growth", "PES"), col=c("red","blue"), lty=c(1),
       inset=c(0,1), xpd=TRUE, horiz=TRUE, bty="n"
       )
...
pred4 <- as.ts(pred4, start=c(2010,3), end = c(2021,3), frequency = 4)
ts.plot(pred4)
...
ts.plot(ts(Data[20:63,1],frequency = 4, start = c(2010,3)),
          ts(pred2,frequency = 4, start = c(2010,3)), # KÜRZEN!
          ts(pred4,frequency = 4, start = c(2010,3)),
    col = c("red", "blue", "black"),
    xlab = "Year", 
    ylab = "Search Frequency (Wirtschaftskrise)", 
    lwd = 1)
legend("bottomright", c("GDP growth", "CCI", "CCI & AR(1)"), col=c("red","blue","black"), lty=c(1),
       inset=c(0,1), xpd=TRUE, horiz=TRUE, bty="n"
       )
...
pred5 <- as.ts(pred5, start=c(2010,3), end = c(2021,3), frequency = 4)
#ts.plot(pred5)
...
ts.plot(ts(Data[20:63,1],frequency = 4, start = c(2010,3)),
          ts(pred5,frequency = 4, start = c(2010,3)),
    col = c("red", "blue"),
    xlab = "Year", 
    ylab = "Search Frequency (Wirtschaftskrise)", 
    lwd = 1)
legend("bottomright", c("GDP growth", "PES & AR(1)"), col=c("red","blue"), lty=c(1),
       inset=c(0,1), xpd=TRUE, horiz=TRUE, bty="n"
       )
...
pred6 <- as.ts(pred6, start=c(2010,3), end = c(2021,3), frequency = 4)
ts.plot(pred6)
